...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  await deploy('BaseRegistrarImplementation', {
    from: deployer,
    args: [registry.address, namehash.hash('dao')],
    log: true,
  })
  ...
  const tx2 = await root.connect(await ethers.getSigner(owner)).setSubnodeOwner('0x' + keccak256('dao'), registrar.address)
  console.log(`Setting owner of dao node to registrar on root (tx: ${tx2.hash})...`)
  await tx2.wait()
