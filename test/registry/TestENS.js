...
contracts.forEach(function ([ENS, lang]) {
    contract('ENS ' + lang, function (accounts) {
        ...
        it('should allow the creation of subnodes', async () => {
            let result = await ens.setSubnodeOwner('0x0', sha3('dao'), accounts[1], {from: accounts[0]});

            assert.equal(await ens.owner(namehash.hash('dao')), accounts[1]);
        ...
        it('should prohibit subnode creation by non-owners', async () => {
            await exceptions.expectFailure(ens.setSubnodeOwner('0x0', sha3('dao'), accounts[1], {from: accounts[1]}));
        });
    });
});
