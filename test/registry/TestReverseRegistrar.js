...
contract('ReverseRegistrar', function(accounts) {
  ...
  let registrar, resolver, dns, nameWrapper, dummyOwnable, defaultResolver
  ...
  beforeEach(async () => {
    ...
    registrar = await ReverseRegistrar.new(dns.address)
    ...
    dummyOwnable = await ReverseRegistrar.new(dns.address)
  ...
  describe('claim', () => {
    it('allows an account to claim its address', async () => {
      ...
      assert.equal(await dns.owner(node), accounts[1])
    })
  ...
  describe('claimForAddr', () => {
    ...
    it('allows an authorised account to claim a different address', async () => {
      await dns.setApprovalForAll(accounts[0], true, { from: accounts[1] })
      await registrar.claimForAddr(
        accounts[1],
        accounts[2],
        resolver.address,
      ...
      assert.equal(await dns.owner(node2), accounts[2])
    })
    ...
    it('allows an owner() of a contract to claim the reverse node of that contract', async () => {
      ...
      assert.equal(await dns.owner(dummyOwnableReverseNode), accounts[0])
    })
  })
  ...
  describe('claimWithResolver', () => {
    it('allows an account to specify resolver', async () => {
      ...
      assert.equal(await dns.owner(node), accounts[1])
      assert.equal(await dns.resolver(node), accounts[2])
    })
  ...
  describe('setName', () => {
    it('sets name records', async () => {
      ...
      assert.equal(await dns.resolver(node), defaultResolver.address)
  ...
  describe('setNameForAddr', () => {
    it('allows controller to set name records for other accounts', async () => {
      ...
      assert.equal(await dns.resolver(node2), resolver.address)
