...
contract('TestRegistrar', function (accounts) {
    ...
    beforeEach(async () => {
        ...
        dns = await DNS.new();
        registrar = await TestRegistrar.new(dns.address, '0x0');
    ...
    it('registers names', async () => {
        ...
        assert.equal(await dns.owner(node), accounts[0]);
    });

    it('forbids transferring names within the test period', async () => {
        await registrar.register(sha3('dao'), accounts[1], {from: accounts[0]});
        await exceptions.expectFailure(registrar.register(sha3('dao'), accounts[0], {from: accounts[0]}));
    });
