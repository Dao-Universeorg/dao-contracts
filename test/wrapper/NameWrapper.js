...
describe('Name Wrapper', () => {
  ...
  before(async () => {
    ...
    DnsRegistry = await deploy('DNSRegistry')
    DnsRegistry2 = DnsRegistry.connect(signers[1])
    DnsRegistryH = DnsRegistry.connect(signers[2])
    ...
    // setup .dao
    await DnsRegistry.setSubnodeOwner(
      ROOT_NODE,
      labelhash('dao'),
      BaseRegistrar.address,
    )
    ...
    // setup .xyz
    await DnsRegistry.setSubnodeOwner(ROOT_NODE, labelhash('xyz'), account)

    //make sure base registrar is owner of eth TLD
    expect(await DnsRegistry.owner(namehash('dao'))).to.equal(
      BaseRegistrar.address,
    )
  })
  ...
  describe('wrap()', () => {
    it('Wraps a name if you are the owner', async () => {
      ...
      await DnsRegistry.setApprovalForAll(NameWrapper.address, true)
    ...
    it('Allows specifying resolver', async () => {
      ...
      expect(await DnsRegistry.resolver(namehash('xyz'))).to.equal(account2)
    })
    ...
    it('Cannot wrap a name if the owner has not authorised the wrapper with the DNS registry.', async () => {
    ...
    it('Allows an account approved by the owner on the DNS registry to wrap a name.', async () => {
      const labelHash = labelhash('abc')

      // setup .abc with account2 as owner
      await DnsRegistry.setSubnodeOwner(ROOT_NODE, labelHash, account2)
      // allow account to deal with all account2's names
      await DnsRegistry2.setApprovalForAll(account, true)
      await DnsRegistry2.setApprovalForAll(NameWrapper.address, true)

      //confirm abc is owner by account2 not account 1
      expect(await DnsRegistry.owner(namehash('abc'))).to.equal(account2)
      // wrap using account
    ...
    it('Does not allow anyone else to wrap a name even if the owner has authorised the wrapper with the DNS registry.', async () => {
      const labelHash = labelhash('abc')
    ...
    it('Does not allow wrapping .dao 2LDs.', async () => {
      const label = 'wrapped'
      const labelHash = labelhash(label)
      await BaseRegistrar.register(labelHash, account, 84600)
      await DnsRegistry.setApprovalForAll(NameWrapper.address, true)
      await expect(
        NameWrapper.wrap(encodeName('wrapped.dao'), account2, EMPTY_ADDRESS),
    ...
    it('Can re-wrap a name that was reassigned by an unwrapped parent', async () => {
      ...
      expect(await DnsRegistry.owner(namehash('sub.xyz'))).to.equal(account2)
      ...
      expect(await DnsRegistry.owner(nameHash)).to.equal(NameWrapper.address)
    })
    ...
    it('Rewrapping a previously wrapped unexpired name retains PCC', async () => {
      const label = 'test'
      const labelHash = labelhash(label)
      const wrappedTokenId = namehash(label + '.dao')
      const subLabel = 'sub'
      const subLabelHash = labelhash(subLabel)
      const subWrappedTokenId = namehash(`${subLabel}.${label}.dao`)
  ...
  describe('unwrap()', () => {
    it('Allows owner to unwrap name', async () => {
      ...
      //Transfers ownership in the DNS registry to the target address.
      expect(await DnsRegistry.owner(namehash('unwrapped.xyz'))).to.equal(
        account,
      )
    })
    ...
    it('Allows an account authorised by the owner on the NFT Wrapper to unwrap a name', async () => {
      ...
      // setup .abc with account2 as owner
      await DnsRegistry.setSubnodeOwner(ROOT_NODE, labelHash, account)
    ...
    it('Does not allow an account authorised by the owner on the DNS registry to unwrap a name', async () => {
      const labelHash = labelhash('abc')
    ...
    it('Will not unwrap .dao 2LDs.', async () => {
      ...
      const ownerOfWrappedETH = await NameWrapper.ownerOf(
        namehash('unwrapped.dao'),
      )
      expect(ownerOfWrappedETH).to.equal(account)
      await expect(
        NameWrapper.unwrap(namehash('dao'), labelhash('unwrapped'), account),
      ).to.be.revertedWith('IncompatibleParent()')
    })
    ...
    it('Will allow to unwrap a name with the CANNOT_UNWRAP fuse burned if expired', async () => {
      ...
      expect(await DnsRegistry.owner(namehash('sub.awesome.dao'))).to.equal(
        NameWrapper.address,
      )
    ...
    it('Will not allow to unwrap a name with the CANNOT_UNWRAP fuse burned if not expired', async () => {
      ...
      await expect(
        NameWrapper.unwrap(namehash('abc.dao'), labelhash('sub'), account),
      ).to.be.revertedWith(`OperationProhibited("${namehash('sub.abc.dao')}")`)
    })
  ...
  describe('wrapETH2LD()', () => {
    ...
    const labelHash = labelhash(label)
    const nameHash = namehash(label + '.dao')
    ...
    it('Cannot wrap a name if the owner has not authorised the wrapper with the .dao registrar.', async () => {
      await BaseRegistrar.register(labelHash, account, 84600)
      await expect(
    ...
    it('Allows specifying resolver', async () => {
      ...
      expect(await DnsRegistry.resolver(nameHash)).to.equal(account2)
    })

    it('Can re-wrap a name that was wrapped has already expired on the .dao registrar', async () => {
      const DAY = 60 * 60 * 24
      ...
      await expect(tx)
        .to.emit(NameWrapper, 'NameUnwrapped')
        .withArgs(namehash('wrapped2.dao'), EMPTY_ADDRESS)
      await expect(tx)
        .to.emit(NameWrapper, 'TransferSingle')
        .withArgs(account2, account, EMPTY_ADDRESS, nameHash, 1)
      await expect(tx)
        .to.emit(NameWrapper, 'NameWrapped')
        .withArgs(
          namehash('wrapped2.dao'),
          encodeName('wrapped2.dao'),
          account2,
          PARENT_CANNOT_CONTROL,
          0,
        )
    ...
    it('correctly reports fuses for a name that has expired and been rewrapped more permissively', async () => {
      ...
      ;[, fuses] = await NameWrapper.getData(namehash('sub.wrapped2.dao'))
      ...
      ;[, fuses, expiry] = await NameWrapper.getData(namehash('wrapped2.dao'))
      expect(fuses).to.equal(PARENT_CANNOT_CONTROL)
    ...
    it('Allows an account approved by the owner on the .dao registrar to wrap a name.', async () => {
      await BaseRegistrar.register(labelHash, account, 84600)
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
    ...
    it('Can wrap a name even if the controller address is different to the registrant address.', async () => {
      await BaseRegistrar.register(labelHash, account, 84600)
      await DnsRegistry.setOwner(nameHash, account2)
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
  ...
  describe('unwrapETH2LD()', () => {
    ...
    it('Allows the owner to unwrap a name.', async () => {
      ...
      expect(await NameWrapper.ownerOf(namehash('unwrapped.dao'))).to.equal(
        account,
      )
      await NameWrapper.unwrapETH2LD(labelHash, account, account)
      // transfers the controller on the .dao registrar to the target address.
      expect(await DnsRegistry.owner(namehash('unwrapped.dao'))).to.equal(
        account,
      )
      //Transfers the registrant on the .dao registrar to the target address
    ...
    it('emits Unwrap event', async () => {
      ...
      await expect(tx)
        .to.emit(NameWrapper, 'NameUnwrapped')
        .withArgs(namehash('unwrapped.dao'), account)
    })
    ...
    it('Does not allows an account authorised by the owner on the DNS registrar to unwrap a name', async () => {
      await BaseRegistrar.register(labelHash, account, 84600)
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
      await BaseRegistrar.setApprovalForAll(account2, true)
      await NameWrapper.wrapETH2LD(
    ...
    it('Does not allow anyone else to unwrap a name even if the owner has authorised the wrapper with the DNS registry.', async () => {
      await BaseRegistrar.register(labelHash, account, 84600)
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
      await DnsRegistry.setApprovalForAll(account2, true)
      await NameWrapper.wrapETH2LD(
        label,
        account,
        CAN_DO_EVERYTHING,
  ...
  describe('setUpgradeContract()', () => {
    ...
    it('Will setApprovalForAll for the upgradeContract addresses in the registrar and registry to true', async () => {
      ...
      expect(
        await DnsRegistry.isApprovedForAll(
          NameWrapper.address,
          NameWrapperUpgraded.address,
        ),
      ).to.equal(false)
    ...
    it('Will setApprovalForAll for the old upgradeContract addresses in the registrar and registry to false', async () => {
      ...
      expect(
        await DnsRegistry.isApprovedForAll(NameWrapper.address, DUMMY_ADDRESS),
      ).to.equal(true)
    ...
    it('Will not setApprovalForAll for the new upgrade address if it is the address(0)', async () => {
      ...
      expect(
        await DnsRegistry.isApprovedForAll(NameWrapper.address, ZERO_ADDRESS),
      ).to.equal(false)
    })
  })
  ...
  describe('upgradeETH2LD()', () => {
    ...
    it('Upgrades a .dao name if sender is owner', async () => {
      await BaseRegistrar.register(labelHash, account, 84600)
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
      ...
      expect(await DnsRegistry.owner(nameHash)).to.equal(
        NameWrapperUpgraded.address,
      )
    ...
    it('Upgrades a .dao name if sender is authorised by the owner', async () => {
      await BaseRegistrar.register(labelHash, account, 84600)
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
      await NameWrapper.setApprovalForAll(account2, true)
      await NameWrapper.wrapETH2LD(
    ...
    it('Will allow you to set the resolver on upgrade.', async () => {
      ...
      const resolver = await DnsRegistry.resolver(nameHash)

      expect(resolver).to.equal(DUMMY_ADDRESS)
    })
    it('Does not allow anyone else to upgrade a name even if the owner has authorised the wrapper with the DNS registry.', async () => {
      await BaseRegistrar.register(labelHash, account, 84600)
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
      await DnsRegistry.setApprovalForAll(NameWrapper.address, true)
  ...
  describe('upgrade()', () => {
    ...
    it('Allows owner to upgrade name', async () => {
      ...
      const ownerOfWrapped = await NameWrapper.ownerOf(
        namehash('to-upgrade.wrapped2.dao'),
      )
      ...
      expect(
        await DnsRegistry.owner(namehash('to-upgrade.wrapped2.dao')),
    ...
    it('upgrades a name if sender is authroized by the owner', async () => {
      ...
      expect(await DnsRegistry.owner(namehash('to-upgrade.xyz'))).to.equal(
        NameWrapperUpgraded.address,
      )
    ...
    it('Will pass fuses and expiry to the upgradedContract without any changes.', async () => {
      const name = 'to-upgrade.wrapped2.dao'
      await DnsRegistry.setApprovalForAll(NameWrapper.address, true)
      await BaseRegistrar.setApprovalForAll(NameWrapper.address, true)
      await BaseRegistrar.register(labelHash, account, 84600)
      await NameWrapper.wrapETH2LD(
      ...
      expect(await DnsRegistry.owner(namehash(name))).to.equal(
        NameWrapperUpgraded.address,
      )
    ...
    it('Will burn the token of the name in the NameWrapper contract when upgraded, but keep expiry and fuses', async () => {
      ...
      expect(
        await NameWrapper.ownerOf(namehash('to-upgrade.wrapped2.dao')),
    ...
    it('reverts if called twice by the original owner', async () => {
      ...
      await expect(
        ...
        `Unauthorised("${namehash('to-upgrade.wrapped2.dao')}", "${account}")`,
      )
    })
  ...
  describe('setFuses()', () => {
    ...
    const wrappedTokenId = namehash('fuses.dao')
    ...
    it('cannot burn PARENT_CANNOT_CONTROL', async () => {
      ...
      await expect(
        NameWrapper.setFuses(namehash('sub.abc.dao'), PARENT_CANNOT_CONTROL),
      ).to.be.revertedWith(
        `Unauthorised("${namehash('sub.abc.dao')}", "${account}")`,
      )
    })
    ...
    it('Will not allow burning fuses of subdomains if CANNOT_UNWRAP has not been burned', async () => {
      ...
      await expect(
        NameWrapper.setFuses(namehash('sub.abc.dao'), CANNOT_TRANSFER),
      ).to.be.revertedWith(
        `OperationProhibited("0x227ab743709550f33d6cb7559e459aa75b799b0f441d950fac5b12a7907d0b19")`,
      )
    })

    it('Will not allow burning fuses of .dao names unless CANNOT_UNWRAP is also burned.', async () => {
      await BaseRegistrar.register(tokenId, account, 84600)
    ...
    it('can set fuses and burn transfer', async () => {
      ...
      const wrappedTokenId = namehash('fuses3.dao')
    ...
    it('can set fuses and burn canCreateSubdomains', async () => {
      ...
      expect(
        await DnsRegistry.owner(namehash('creatable.fuses2.dao')),
      ).to.equal(NameWrapper.address)

      expect(
        await NameWrapper.ownerOf(namehash('creatable.fuses2.dao')),
      ).to.equal(account)
      ...
      await expect(
        ...
        `OperationProhibited("${namehash('uncreatable.fuses2.dao')}")`,
      )
    })
  })
  ...
  describe('setChildFuses', () => {
    ...
    const tokenId = labelhash(label)
    const wrappedTokenId = namehash(`${label}.dao`)
    const subWrappedTokenId = namehash(`sub.${label}.dao`)
    ...
    it('Allows parent owners to set fuses/expiry', async () => {
      ...
      ;[, fuses, expiry] = await NameWrapper.getData(namehash('sub.fuses.dao'))
    ...
    it('Does not allow non-parent owners to set child fuses', async () => {
      const subWrappedTokenId = namehash('sub.fuses.dao')
      await registerSetupAndWrapName(
        'fuses',
        account,
        CANNOT_UNWRAP,
    ...
    it('Allows .dao owners to set their fuses/expiry', async () => {
    ...
    it('Normalises expiry to the .dao registrar expiry', async () => {
      await registerSetupAndWrapName('fuses', account, 0, 0)
      ...
      await NameWrapper.setChildFuses(namehash('dao'), tokenId, 0, MAX_EXPIRY)
      // expiry in the .dao registrar
      const expectedExpiry = await BaseRegistrar.nameExpires(tokenId)
      ;[, , expiry] = await NameWrapper.getData(wrappedTokenId)
      expect(expiry).to.equal(expectedExpiry)
    })
    ...
    it('Normalises expiry to the old expiry', async () => {
      ...
      await NameWrapper.setChildFuses(namehash('dao'), tokenId, 0, 500)
      ;[, , expiry] = await NameWrapper.getData(wrappedTokenId)
      expect(expiry).to.equal(1000)
    })
  ...
  describe('setSubnodeOwner()', async () => {
    ...
    it('Can be called by the owner of a name and sets this contract as owner on the DNS registry.', async () => {
      expect(await NameWrapper.ownerOf(wrappedTokenId)).to.equal(account)
      await DnsRegistry.setApprovalForAll(NameWrapper.address, true)
      await NameWrapper.setSubnodeOwner(
        wrappedTokenId,
      ...
      expect(await DnsRegistry.owner(namehash(`sub.${label}.dao`))).to.equal(
        NameWrapper.address,
      )

      expect(await NameWrapper.ownerOf(namehash(`sub.${label}.dao`))).to.equal(
        account,
      )
    })
    it('Can be called by an account authorised by the owner.', async () => {
    ...
    it('Fuses cannot be burned if the name does not have PARENT_CANNOT_CONTROL burned', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("${namehash(`${label2}.${label}.dao`)}")`,
      )
    })
    ...
    it('Unwrapping within an external contract does not create any state inconsistencies', async () => {
      ...
      // set self as sub.test.dao owner
      await NameWrapper.setSubnodeOwner(
        namehash('test.dao'),
        'sub',
        account,
  ...
  describe('setSubnodeRecord()', async () => {
    ...
    it('Does not allow fuses to be burned if PARENT_CANNOT_CONTROL is not burned.', async () => {
      ...
      await expect(
        ...
        `OperationProhibited("${namehash(`sub.${label}.dao`)}")`,
      )
    })
    ...
    it('Fuses will remain 0 if expired', async () => {
      ...
      const [, fuses] = await NameWrapper.getData(namehash(`sub.${label}.dao`))
      expect(fuses).to.equal(0)
    })
    ...
    it('Sets the appropriate values on the DNS registry', async () => {
      await NameWrapper.setSubnodeRecord(
        wrappedTokenId,
        'sub',
        account2,
      ...
      const node = namehash(`sub.${label}.dao`)

      expect(await DnsRegistry.owner(node)).to.equal(NameWrapper.address)
      expect(await DnsRegistry.resolver(node)).to.equal(resolver)
      expect(await DnsRegistry.ttl(node)).to.equal(100)
    })
  ...
  describe('setRecord', () => {
    ...
    it('Performs the appropriate function on the DNS registry and Wrapper', async () => {
      await NameWrapper.setRecord(wrappedTokenId, account2, account, 50)

      expect(await NameWrapper.ownerOf(wrappedTokenId)).to.equal(account2)
      expect(await DnsRegistry.resolver(wrappedTokenId)).to.equal(account)
      expect(await DnsRegistry.ttl(wrappedTokenId)).to.equal(50)
    })
  ...
  describe('setResolver', () => {
    ...
    it('Performs the appropriate function on the DNS registry.', async () => {
      expect(await DnsRegistry.resolver(wrappedTokenId)).to.equal(EMPTY_ADDRESS)
      await NameWrapper.setResolver(wrappedTokenId, account2)
      expect(await DnsRegistry.resolver(wrappedTokenId)).to.equal(account2)
    })
  ...
  describe('setTTL', () => {
    ...
    it('Performs the appropriate function on the DNS registry.', async () => {
      expect(await DnsRegistry.ttl(wrappedTokenId)).to.equal(EMPTY_ADDRESS)
      await NameWrapper.setTTL(wrappedTokenId, 100)
      expect(await DnsRegistry.ttl(wrappedTokenId)).to.equal(100)
    })
  ...
  describe('onERC721Received', () => {
    ...
    it('Reverts if called by anything other than the DNS registrar address', async () => {
      await BaseRegistrar.register(tokenId, account, 84600)
    ...
    it('Reverts if CANNOT_UNWRAP is not burned and attempts to burn other fuses', async () => {
      await BaseRegistrar.register(tokenId, account, 84600)
      await DnsRegistry.setOwner(wrappedTokenId, account2)
    ...
    it('Allows burning other fuses if CAN_UNWRAP has been burnt', async () => {
      ...
      expect(await DnsRegistry.owner(wrappedTokenId)).to.equal(
        NameWrapper.address,
      )
      expect(await NameWrapper.ownerOf(wrappedTokenId)).to.equal(account)
    ...
    it('Sets the controller in the DNS registry to the wrapper contract', async () => {
      await BaseRegistrar.register(tokenId, account, 84600)
  ...
  describe('getData', () => {
    ...
    const subLabel = 'sub'
    const subLabelHash = labelhash(subLabel)
    const subNameHash = namehash(`${subLabel}.${label}.dao`)
    const subSubLabel = 'subsub'
    const subSubLabelhash = labelhash(subSubLabel)
    const subSubNameHash = namehash(`${subSubLabel}.${subLabel}.${label}.dao`)
  ...
  describe('registerAndWrapETH2LD', () => {
    ...
    it('Does not allows fuse to be burned if CANNOT_UNWRAP has not been burned.', async () => {
      ...
      ).to.be.revertedWith(`OperationProhibited("${namehash(label + '.dao')}")`)
    })
  ...
  describe('MetadataService', () => {
    it('uri() returns url', async () => {
      expect(await NameWrapper.uri(123)).to.equal('https://dao.domains')
    })
  ...
  describe('NameWrapper.names preimage dictionary', () => {
    it('Does not allow manipulating the preimage db by manually setting owner as NameWrapper', async () => {
      ...
      const sub1Domain = sub1Label + '.' + label + '.dao' // sub1.base.dao
      ...
      expect(await DnsRegistry.owner(wrappedSub1TokenId)).to.equal(
        NameWrapper.address,
      )
      ...
      expect(await DnsRegistry.owner(wrappedSub1TokenId)).to.equal(hacker)
      ...
      const sub2Domain = sub2Label + '.' + sub1Domain // sub2.sub1.base.dao
      const wrappedSub2TokenId = namehash(sub2Domain)
      await DnsRegistryH.setSubnodeOwner(
        wrappedSub1TokenId,
        sub2LabelHash,
        NameWrapper.address,
      )
      expect(await DnsRegistry.owner(wrappedSub2TokenId)).to.equal(
        NameWrapper.address,
      )

      // the hacker re-wraps the sub1node
      await DnsRegistryH.setApprovalForAll(NameWrapper.address, true)
      ...
      const sub3Domain = sub3Label + '.' + sub2Domain // dao.sub2.sub1.base.dao
  ...
  describe('ERC1155 additional tests', () => {
    ...
    it('Approval on the Wrapper does not give permission to wrap the .dao name', async () => {
    ...
    it('Approval on the Wrapper does not give permission to wrap a non .dao name', async () => {
      expect(await DnsRegistry.owner(namehash('xyz'))).to.equal(account)
      await NameWrapper.setApprovalForAll(hacker, true)
      await DnsRegistry.setApprovalForAll(NameWrapper.address, true)
      await expect(
