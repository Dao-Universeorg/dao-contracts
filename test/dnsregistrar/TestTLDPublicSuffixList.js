...
contract('TLDPublicSuffixList', function(accounts) {
  ...
  it('treats all TLDs as public suffixes', async function() {
    assert.equal(await suffixList.isPublicSuffix(utils.hexEncodeName('dao')), true);
  ...
  it('treats non-TLDs as non-public suffixes', async function() {
    ...
    assert.equal(await suffixList.isPublicSuffix(utils.hexEncodeName('foo.dao')), false);
    assert.equal(await suffixList.isPublicSuffix(utils.hexEncodeName('a.b.foo.dao')), false);
  });
});
