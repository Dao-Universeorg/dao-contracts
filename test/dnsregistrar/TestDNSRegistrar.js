const DNSRegistry = artifacts.require('./DNSRegistry.sol')
...
contract('DNSRegistrar', function(accounts) {
  ...
  it('allows anyone to claim on behalf of the owner of an DNS name', async function() {
    assert.equal(await registrar.oracle(), dnssec.address)
    assert.equal(await registrar.dns(), dns.address)
    ...
    assert.equal(await dns.owner(namehash.hash('foo.test')), accounts[0])
  })
  ...
  it('allows claims on names that are not TLDs', async function() {
    ...
    assert.equal(await dns.owner(namehash.hash('foo.co.nz')), accounts[0])
  })
  ...
  it('allows anyone to update a DNSSEC referenced name', async function() {
    ...
    assert.equal(await dns.owner(namehash.hash('foo.test')), accounts[1])
  })
  ...
  it('allows the owner to claim and set a resolver', async () => {
    ...
    assert.equal(await dns.owner(namehash.hash('foo.test')), accounts[0])
    assert.equal(await dns.resolver(namehash.hash('foo.test')), accounts[1])
  })
