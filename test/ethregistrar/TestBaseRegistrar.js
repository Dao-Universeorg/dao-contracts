...
contract('BaseRegistrar', function (accounts) {
	...
	before(async () => {
		...
		registrar = await BaseRegistrar.new(ens.address, namehash.hash('dao'), {from: ownerAccount});
	...
	it('should allow new registrations', async () => {
		...
		assert.equal(await ens.owner(namehash.hash("newname.dao")), registrantAccount);
	...
	it('should allow registrations without updating the registry', async () => {
		...
		var block = await web3.eth.getBlock(tx.receipt.blockHash);
		assert.equal(await ens.owner(namehash.hash("silentname.dao")), ZERO_ADDRESS);
	...
	it('should permit the owner to reclaim a name', async () => {
		await ens.setSubnodeOwner(ZERO_HASH, sha3("dao"), accounts[0]);
		await ens.setSubnodeOwner(namehash.hash("dao"), sha3("newname"), ZERO_ADDRESS);
		assert.equal(await ens.owner(namehash.hash("newname.dao")), ZERO_ADDRESS);
		await ens.setSubnodeOwner(ZERO_HASH, sha3("dao"), registrar.address);
	...
	it('should allow the owner to set a resolver address', async () => {
		...
		assert.equal(await ens.resolver(namehash.hash('dao')), accounts[1]);
	});
});
