...
import "../registry/DNSRegistry.sol";
...
/**
 * @dev An DNS registrar that allows the owner of a DNS name to claim the
 *      corresponding name in DNS.
 */
...
contract DNSRegistrar is IDNSRegistrar, IERC165 {
    ...
    /**
     * @dev This contract's owner-only functions can be invoked by the owner of the DNS root.
     */
    modifier onlyOwner() {
        Root root = Root(dns.owner(bytes32(0)));
    ...
    function proveAndClaim(
        ...
        dns.setSubnodeOwner(rootNode, labelHash, addr);
    }
    ...
    function proveAndClaimWithResolver(
        ...
        if (addr != address(0)) {
            ...
            dns.setSubnodeRecord(rootNode, labelHash, owner, resolver, 0);
        }
    }
        ...
        returns (bytes32 node)
    {
        ...
        if (owner != address(this)) {
            if (parentNode == bytes32(0)) {
                ...
                dns.setSubnodeOwner(parentNode, label, address(this));
            }
        }
        return node;
    }
